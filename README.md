...
- **Microphone capture** — 16-bit little-endian mono PCM at 16 kHz off the device
  ...
  below.
- **Speech to text** — a streaming zipformer (en, 20M, int8) through `sherpa_onnx`
  on a dedicated background isolate, behind `SttEngine`. See _Speech to text_ below.
- **Voice input, wired to the screen** — a microphone button beside the inquiry
  field: tap it, speak, and the transcript builds into the inquiry the agent then
  answers. It resolves the **real** recogniser or `null` — never the fake — for the
  reason the answer path does. See _Voice input and the work order_ below.
- **Dynamic form auto-fill** — the agent records the work order (fault code,
  replacement parts, technician hours, safety checkpoints) by *calling a tool*
  rather than by having JSON scraped out of its prose, and can ask which of several
  values it should have written. A field the technician typed is never overwritten;
  the agent's value is offered beside it instead. See _Voice input and the work
  order_ below.
...
Task 2.3 added the microphone beside the inquiry field and the work-order panel
below the answer. Both are described under _Voice input and the work order_; what
...
See _Voice input and the work order_ below.
