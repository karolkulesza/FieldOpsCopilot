...
///
/// The layout is the one the golden suite pins — a system preamble,
...
/// (or dictates something transcribed as) `[MANUAL DOCUMENT]` can
...
/// the spellings someone enumerated: one extra space
/// (`[MANUAL  DOCUMENT]`) broke the original, and a leading space, a tab and a
/// newline did the same. Widening the pattern to absorb whitespace would still have left
...
///
/// The residual is named rather than hidden, because it is easier to claim more
/// than the code does than to state the boundary. Delimiters *outside*
...
/// names. An earlier version carried `So` for U+23A1 through this doc, the
/// README and a test
/// comment before anyone ran `unicodedata` on it — it is `Sm`,
...
/// only while this block was the last thing in the prompt. The agent
/// loop appends tool-call and tool-result blocks after it, so it no longer is.
///
/// **Documents are capped.** A single entry measures at a ~400-token grounded
/// prompt against a 2B-parameter model, and the router can return one
...
class PromptCompiler {
  ...
  /// constructor used to `assert` instead, which was the wrong mechanism:
  /// an `assert` is compiled out in release, so it
  ...
  /// no test could bind. One mechanism, live in every build.
  ...
  ///
  /// **"Do not call any tool" was narrowed to the lookup when the second tool
  /// arrived — the sentence had to change because the *registry* did.**
  ...
  String compile(RetrievalResult result) {
    ...
    // to break into. The agent loop appends `[TOOL CALL]` / `[TOOL RESULT]`
    // blocks after the whole compiled prompt for the second model turn, so this
    // block is no longer last, and the note once left here ("this needs
  ...
  /// A single document's header. A lone document keeps the exact
  ...
  ///
  /// Field order is title first, then the procedure and
  /// what it needs. `Required Parts` is emitted before `Required Tools` because
  /// the parts line is what the preamble's tool instruction acts on —
  ...
  /// exactly the enumeration this rewrite exists to stop relying on — the
  /// previous version claimed to handle
